...
@export var lift_height := 0.55
@export var settle_duration := 0.18
@export var min_spins := 2
@export var max_spins := 4

@export var mirror_numbers := true
@export var body_scale := 0.68
@export var label_font_size := 82
@export var label_pixel_size := 0.008
...
func roll() -> void:
	...
	_target_value = _rng.randi_range(1, 20)
...
func _build_mesh() -> void:
	...
	for f: Array in faces:
		var a: Vector3 = verts[int(f[0])]
		var b: Vector3 = verts[int(f[1])]
		var c: Vector3 = verts[int(f[2])]
		_add_tri(st, a, b, c)
...
func _build_numbers() -> void:
	...
	var face_count: int = mini(20, mdt.get_face_count())
